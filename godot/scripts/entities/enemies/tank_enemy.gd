...
## Tank enemy type - high health, low speed
## Stats are configured via EnemyDefinition resource (tank_enemy_definition.tres)
...
func _on_enemy_ready() -> void:
	...
	if health == 50 and speed == 50.0:  # Still at default values
		var definition := GameConfig.get_enemy_definition("tank")
		if definition:
